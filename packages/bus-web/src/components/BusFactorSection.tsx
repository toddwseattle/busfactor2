import type { BusfactorReportSection, FileContributorReport } from "bus-lib";
...
const ContributorActivity = ({
  ...
  <ul className="flex min-w-64 flex-col gap-2">
    {contributors.map((contributor) => (
      <li
        className="flex flex-wrap items-center gap-x-2 gap-y-1 text-sm"
        key={contributor.author}
      >
        <span className="font-medium text-[#34214f]">{contributor.author}</span>
        <span className="text-[#62576f]">
          {formatPercent(contributor.contributionPercent)}
        </span>
        <span className="rounded-md bg-[#f6f2eb] px-2 py-0.5 text-xs font-semibold text-[#62576f]">
          {formatEditLabel(contributor.editCount)}
        </span>
      </li>
    ))}
  </ul>
        ...
        <table className="min-w-full border-collapse text-left text-sm">
          ...
          <thead>
            <tr className="border-b border-[#d8d0c4] text-xs uppercase tracking-[0.14em] text-[#62576f]">
              ...
              <th className="min-w-72 px-3 py-3 font-semibold" scope="col">
          ...
          <tbody className="divide-y divide-[#ece4d8]">
            {section.files.map((file) => (
              <tr key={file.path}>
                ...
                <td className="px-3 py-4 align-top">
                  <ContributorActivity contributors={file.contributors} />
                </td>
              </tr>
            ))}
          </tbody>
